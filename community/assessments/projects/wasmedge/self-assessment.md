...
<!-- cSpell:ignore Ying Shen Hsieh ibmibmibm hydai wasmedge wasi unvalidated spectest segv kwasm crun -->
...
| Languages | C++ |
| SBOM | The software bill of materials for WasmEdge can be found in [sbom.tar.gz](https://github.com/WasmEdge/WasmEdge/releases/download/0.14.1/sbom.tar.gz) within each release. |
...
Therefore, we can ascertain that when WasmEdge executes any WebAssembly instruction, as long as it is not executing call imported functions, the executed instruction will not affect the host environment (the environment where WasmEdge is executed).
...
For example, by adding support for WebAssembly functionality in a software product, we can support machine learning interfaces like [WASI-NN](https://github.com/WebAssembly/wasi-nn). This makes it easier for developers to support machine learning backends such as OpenVINO or PyTorch.
...
* WasmEdge Validator
  ...
  + Checks for semantic correctness, which ensure that the AST read by the Loader is valid, are handled according to the [WebAssembly validation](https://webassembly.github.io/spec/core/valid/index.html).
...
* WasmEdge Engine
  ...
  + Each module can only see the virtual resources and virtual memories provided by the WasmEdge Engine. This approach allows the WasmEdge Engine to effectively manage OS resources and memories. On the same host, each executing wasm program sees an independent set of OS resources and memory space, managed by the WasmEdge Engine.
...
* Regarding the issue of DoS, if users execute relatively high-load programs on these resource-constrained devices leading to DoS, it is not within our focus scope. We believe that if DoS is caused by improper design of the WasmEdge engine or executor, then that is our focus goal.
...
* If users want to prevent host files from being modified, they could add the :readonly flag in the option, like `--dir guest_path:host_path:readonly`. The WasmEdge runtime will manage the filesystem usage to only grant read-only access to the wasm program.
...
* Development Pipeline
  ...
  + The source of the official WebAssembly spec test is at [WebAssembly/spec/test](https://github.com/WebAssembly/spec/tree/main/test). We have converted these tests into a format executable by WasmEdge, available at [WasmEdge/wasmedge-spectest](https://github.com/WasmEdge/wasmedge-spectest).
    The entry for these tests on CI is in [build.yml](https://github.com/WasmEdge/WasmEdge/blob/master/.github/workflows/build.yml), and the test results can be seen in [Test WasmEdge Core](https://github.com/WasmEdge/WasmEdge/actions/workflows/build.yml). The logs of the CI tests are publicly available.
...
* Communication Channels
  ...
  + Outbound
    - [WasmEdge Discord server](https://discord.gg/h4KDyB8XTt).
    - **#wasmedge** channel on the [CNCF Slack](https://slack.cncf.io/).
    - The mailing list [wasmedge@googlegroups.com](mailto:wasmedge@googlegroups.com).
...
* Send email to [wasmedge-security@lists.cncf.io](mailto:wasmedge-security@lists.cncf.io)
...
* Please visit the [GitHub Security Advisory of WasmEdge](https://github.com/WasmEdge/WasmEdge/security/advisories/new)
...
* We have a mailing list at <wasmedge@googlegroups.com>, which you can join or browse through this link: <https://groups.google.com/g/wasmedge/>. We plan to send upgrade notifications through this mailing list.
